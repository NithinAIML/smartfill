...
def query_similar_chunks(question, index, chunks, k=3):
    ...
    if question_embedding.shape[1] != index.d:
        raise ValueError(
            f"Embedding dimension {question_embedding.shape[1]} does not match index dimension {index.d}"
        )
...
Answer:"""
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a knowledgeable assistant providing accurate answers based on given context.",
            },
    ...
    for question in questions:
        try:
            ...
            if not similar_chunks:
                ...
                answers[question] = (
                    "Unable to generate answer - no relevant context found"
                )
                continue
