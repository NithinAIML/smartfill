...
class RFPProcessor:
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row in ws.iter_rows(min_row=1):
                ...
                if q_cell and q_cell.value and not q_cell.font.bold:
                    ...
                    if not is_date(question) and len(question.split()) > 3:  # Basic check for sentence-like structure
    ...
    def load_training_qa_pairs(self, excel_path: str):
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row in ws.iter_rows(min_row=1):
                ...
                if q_cell and q_cell.value and not q_cell.font.bold and r_cell and r_cell.value:
                    question = str(q_cell.value).strip()
    ...
    def find_similar_questions(self, question: str, threshold: float = 0.85) -> List[Tuple[str, float]]:
        ...
        for train_q, train_emb in self.training_embeddings.items():
            similarity = np.dot(q_embedding, train_emb) / (np.linalg.norm(q_embedding) * np.linalg.norm(train_emb))
    ...
    def evaluate_context_sufficiency(self, question: str, context: str) -> bool:
        ...
        response = self.openai_client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert at evaluating context sufficiency for answering questions."},
