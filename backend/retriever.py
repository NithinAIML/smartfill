...
def answer_questions_with_rag(questions):
    ...
    for idx, question in enumerate(questions):
        ...
        result = client.query.get("DocumentChunk", ["text", "source"])             .with_near_vector({"vector": q_vector})             .with_limit(3)             .do()

        context_chunks = [item["text"] for item in result["data"]["Get"]["DocumentChunk"]]
        ...
        completion = openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Answer RFP questions using the provided context."},
